...
function findOutlier(a: number[]): number {
	...
	const outlierIsEven: boolean = evenCounter < oddCounter;
	console.log("outlierIsEven", outlierIsEven);
	let outlierNumber: number = 0;

	for (let num of a) {
		if (num % 2 === 0 && outlierIsEven) {
			outlierNumber = num;
			break;
		} else if (num % 2 !== 0 && !outlierIsEven) {
			outlierNumber = num;
			break;
		}
	}
